export default class History {
  ...
  init = () => {
    ...
    this.emitRouteChange({
      current: [{ uri: this.history[this.current]? this.history[this.current] : this.getHashURI(location.href), className: 'current', index: this.current }],
      next: [],
      end: [{ uri: this.history[this.current]? this.history[this.current] : this.getHashURI(location.href), className: 'current', index: this.current }],
    })
  }
  ...
  push = (currentURI, newURI, page) => {
    ...
    this.emitRouteChange({
      current: [{ uri: currentURI, className: 'current', index }, {
        uri: newURI,
        className: 'next',
        index: page
      }],
      next: [{ uri: currentURI, className: 'prev', index }, {
        uri: newURI,
        className: 'current',
        index: page
      }],
  ...
  buildURI = to => {
    ...
    if (/^\/\//i.test(to)) {
      ...
      const hash = this.history[this.current] ? this.history[this.current] : window.location.hash.slice(1).split('?', 1)[0];
        const p = hash.split('/')
      p.pop()
  ...
  restoreHistory = () => {
    const cache = JSON.parse(sessionStorage.getItem((location + '').split('#')[0]) || 'null')
