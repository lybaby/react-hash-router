...
var History = function History(observer) {
  ...
  this.init = function () {
    ...
    _this.emitRouteChange({
      current: [{ uri: _this.history[_this.current] ? _this.history[_this.current] : _this.getHashURI(location.href), className: 'current', index: _this.current }],
      next: [],
      end: [{ uri: _this.history[_this.current] ? _this.history[_this.current] : _this.getHashURI(location.href), className: 'current', index: _this.current }]
    });
  };
  ...
  this.push = function (currentURI, newURI, page) {
    ...
    _this.emitRouteChange({
      current: [{ uri: currentURI, className: 'current', index: index }, {
        uri: newURI,
        className: 'next',
        index: page
      }],
      next: [{ uri: currentURI, className: 'prev', index: index }, {
        uri: newURI,
        className: 'current',
        index: page
      }],
  ...
  this.backward = function (previousURI, currentURI, page) {
    ...
    _this.current = page;
    _this.emitRouteChange({
      current: [{ uri: previousURI, className: 'prev', index: page }, {
        uri: currentURI,
        className: 'current',
        index: index
      }],
      next: [{ uri: previousURI, className: 'current', index: page }, {
        uri: currentURI,
        className: 'next',
        index: index
      }],
  ...
  this.buildURI = function (to) {
    ...
    if (/^\/\//i.test(to)) {
      ...
      var hash = _this.history[_this.current] ? _this.history[_this.current] : window.location.hash.slice(1).split('?', 1)[0];
      var p = hash.split('/');
      p.pop();
  ...
  this.endPath = function () {
    ...
    if (current.index < end.index) {
      _this.emitRouteChange({
        current: [{ uri: current.uri, className: 'current', index: current.index }, {
          uri: end.uri,
          className: 'next',
          index: end.index
        }],
        next: [{ uri: current.uri, className: 'prev', index: current.index }, {
          uri: end.uri,
          className: 'current',
          index: end.index
        }],
      ...
      _this.emitRouteChange({
        current: [{ uri: end.uri, className: 'prev', index: end.index }, {
          uri: current.uri,
          className: 'current',
          index: current.index
        }],
        next: [{ uri: end.uri, className: 'current', index: end.index }, {
          uri: current.uri,
          className: 'next',
          index: current.index
        }],
  ...
  this.restoreHistory = function () {
    var cache = JSON.parse(sessionStorage.getItem((location + '').split('#')[0]) || 'null');
  ...
  this.cacheHistory = function () {
    ...
    sessionStorage.setItem((location + '').split('#')[0], cache);
  };
  ...
  if (history.state === null || !('PAGE' in history.state)) {
    ...
    this.current = history.state.PAGE;
    this.cacheHistory();
  }
};
